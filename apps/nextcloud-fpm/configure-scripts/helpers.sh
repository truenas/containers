...
merge_app_value() {
  ...
  # The current value is piped in and read with the json parser on purpose.
  # php escapes forward slashes when it stores these, and `from_json` rejects
  # the resulting `\/` with "found unknown escape character". The new value is
  # generated by us and has no such escaping, so `from_json` is fine for it.
