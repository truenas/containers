...
| `IX_HARP_INTERNAL_ADDRESS`     | HaRP daemon host:port as seen from Nextcloud (internal); used to register the deploy daemon |  `""`   | `harp:8780`         |
...
| `IX_HARP_COMPUTE_DEVICE`       | Computation device for GPU support (`cpu`, `cuda`, `rocm`)                                  |  `cpu`  | `cuda`              |
