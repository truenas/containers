...
run_post_upgrade_scripts() {
  ...
  if [ -z "$sql_files" ]; then
    up_log "No post-upgrade SQL scripts generated"
    return 0
  fi
  ...
  up_log "Starting temporary PostgreSQL server to run post-upgrade scripts..."
  ...
  up_log "Post-upgrade scripts completed"
}
...
perform_upgrade() {
  ...
  run_post_upgrade_scripts "$new_data_dir" "$sql_files"
